...
const UsersCard: React.FC<UsersCardProps> = ({ avatarUrl, name, latestMessage, time, type }) => {
  return (
    <div className="flex items-center p-4 border-b border-gray-200 relative hover:cursor-pointer">
      ...
      {type === 'chat' && (
        <div className="flex-1">
          ...
          {time && (
            <div className="text-xs text-gray-400 absolute right-4 top-1/2 transform -translate-y-1/2">
              {time}
            </div>
          )}
        </div>
